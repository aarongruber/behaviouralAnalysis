...
for i=1:numel(TR_c) % loop over all arrays of objects passed in
        ...
        if(PolyOrder>0) % do polynomial fit if given order
           ...
           % w = (ds_ok.GroupCount(indx)).^1.5./x.^2; 
            ...
            if(Plt_each_flg)  % if input flag thrown, plot each group independenty in a new fig
                % plot each fit on one figure, and plot the X2 and X coeffs
                if(i==1)
                    figure; h_allOnOne = subplot(2,1,1, 'XScale','log');  % plot all of them on one axis, but segragate according to the quadratic term
                        xlabel(h_allOnOne, 'ITI'); ylabel(param)
                        title(h_allOnOne, 'Fit for each subject, offset in y by the quadratic coeff'); hold on
                    h_allOnOne2 = subplot(2,1,2);  % plot all of them on one axis, but segragate according to the quadratic term
                        xlabel(h_allOnOne2, 'quadratic term'); ylabel('linear term')
                        title(h_allOnOne2, 'terms for polynomial fit; significant coeffs. shown by open symbols'); hold on
                end
                pt_h=plot(h_allOnOne, 10.^x1',ypred ,'b-');   % plot the fit, separate by B for quadratic term
                if(p_w(1) >0); 
                    set(pt_h,'Color','r') % turn the line red if the curvature is upward
                end
                plt1_h = plot(h_allOnOne2, p_w(1), p_w(2),'*'); hold on
                if(nlm_w.Coefficients.pValue(1)<0.05); plot(h_allOnOne2, p_w(1), p_w(2),'<'); end
                if(nlm_w.Coefficients.pValue(2)<0.05); plot(h_allOnOne2, p_w(1), p_w(2),'>'); end
                %if(); end
                
                % plot each fit on a new fiture
    ...
    d(i).f20median80 = quantile([TR_c{i}.InterTrialInterval]./1000, [0.20 0.5 0.8]);
    d(i).numTR = numel(TR);
    d(i).ITI = [TR.InterTrialInterval]./1000;
...
if(PolyOrder > 0)
    ...
    bins = linspace(min(BforHterm), max(BforHterm), 10);
else
    bins = [-1:0.05:1];                   % bins for plotting dist of slopes
    BforHterm = slope(2,:);
end

%[n_terms, n_fits] = size(slope);  % find out how many fits were saved in slope (should probably make slope a cell array to avoid problems); do hist of all if missmatch
%if( numel(grp)<2 || numeel(grp)>n_fits) % if 0 or 1 group, plot all of the coefficients
if( numel(grp)<2)
    [nSlope, bins]=hist(BforHterm,bins);
    stairs(ax(5),bins,nSlope); hold on; set(ax(5),'XLim',[min(BforHterm), max(BforHterm)+eps])
    ...
    for i=1:numel(unique(grp))            % loop over the number of groups
        ...
        slopesForGrp = [d(indxGpIndx).B];
        [nSlope, bins]=hist(slopesForGrp,bins);
        stairs(ax(5),bins,nSlope,'Color',getColor(grp_indx(indxGpIndx(1)),'num')); hold on
        title(ax(5),['tstat: ',num2str(stats.tstat), '; p: ',num2str(P), '; df: ', num2str(stats.df) ])
    end
end
...
if(0)
    ...
    for i=1:numel(d)
        if(~isempty(d(i).B))
            x8 = [x8, d(i).f20median80(2)];
            %x8 = [x8, mean(d(i).ITI)];
            y8 = [y8, d(i).B(2)];        %% NB; for quadratic B=1; for linear x fit x=2
            plot(x8, y8,'o'); hold on
            %plot(d(i).f20median80(2), d(i).B(2),'*'); hold on
        end
    end
    [B,BINT,R,RINT,STATS] = regress(y8', [ones(numel(x8'),1), x8']);
    %[B, STAT2] = robustfit(x8,y8);
    
    xlims = [max(x8), min(x8)];
    plot([xlims],[B(1)+xlims(1)*B(2), B(1)+xlims(2)*B(2)] ,'k:'); hold on
    xlabel('median ITI')
    ylabel('slope')
    title(['for', param,';  R=',num2str(STATS(1)),'; F=',num2str(STATS(2)),'; P=',num2str(STATS(3))])
    
end



%%% some plots of coefficients vs ITI, and # trials. 
% figure
% for i=1:numel(d)
%     if(~isempty(d(i).B))
%         %[i, d(i).numTR, d(i).B(1)]
%         %plot(d(i).numTR, d(i).B(1),'o'); hold on 
%         plot(d(i).f20median80(1), d(i).B(1),'o'); hold on
%         set(gca,'XScale','log')
%     end
%     %d(i).20median80(1), 
% end
